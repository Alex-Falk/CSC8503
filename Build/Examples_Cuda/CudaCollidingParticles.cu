...
void CudaCollidingParticles::ShootParticle(float3 vel, float3 pos)
{
...
///This function could have been a lot simpler, but I wanted nicely compacted dam... >.>
	...
	Particle* tmp_particles = new Particle[num_particles+1];

	Particle bigBall = Particle();
	bigBall._pos = make_float3(0.0f,1.0f,0.0f);

	tmp_particles[num_particles] = bigBall;
