...
__global__
void CollideParticles(float baumgarte_factor, uint num_particles, Particle* particles, Particle* out_particles, uint* grid_cell_start, uint* grid_cell_end, Particle * cpu_particles, int num_CPU_obj)
{
	...
	for (int i = 0; i < num_CPU_obj; ++i) {
		...
		//	printf("%d %f %f %f \n", i, cpu_particles[i]._pos.x, cpu_particles[i]._pos.y, cpu_particles[i]._pos.z);
		//	printf("%d %f %f %f \n", i, cpu_particles[i]._vel.x, cpu_particles[i]._vel.y, cpu_particles[i]._vel.z);
		//}

		float3 newpos = make_float3(cpu_particles[i]._pos.x + 9.6f, cpu_particles[i]._pos.y,cpu_particles[i]._pos.z + 9.6f);

		float bigRadius = 1.0f;
		//Do a quick sphere-sphere test
		float3 ab = newpos - out_p._pos;
		float lengthSq = dot(ab, ab);

		const float diameterSq = ((PARTICLE_RADIUS + bigRadius) * (PARTICLE_RADIUS + bigRadius)) * 0.25f;
		if (lengthSq < diameterSq)
		{
			//We have a collision!
			float len = sqrtf(lengthSq);
			...
			//Direct normal collision (no friction/shear)
			float abnVel = dot(cpu_particles[i]._vel*0.25f - out_p._vel, abn);
			float jn = -(abnVel * (1.f + COLLISION_ELASTICITY));

			//Extra energy to overcome overlap error
			float overlap = bigRadius - len;
			float b = overlap * baumgarte_factor;
...
CudaCollidingParticles::~CudaCollidingParticles()
{
	if (particles_ping)
	{
		...
		gpuErrchk(cudaFree(cpuParticles));
...
void CudaCollidingParticles::UpdateParticles(float dt, vector<Vector3> positions, vector<Vector3> velocities)
{
	...
	const float3 gravity = make_float3(0, -0.08f, 0);
	const uint num_grid_cells = PARTICLE_GRID_SIZE*PARTICLE_GRID_SIZE*PARTICLE_GRID_SIZE;
	...
	int num_CPU_obj = positions.size();

	Particle* cpu_particles = new Particle[num_CPU_obj];

	for (int i = 0; i < num_CPU_obj; ++i) {
		cpu_particles[i]._pos = make_float3(positions[i].x, positions[i].y, positions[i].z);
		cpu_particles[i]._vel = make_float3(velocities[i].x, velocities[i].y, velocities[i].z);
	}

	gpuErrchk(cudaMalloc(&cpuParticles, num_CPU_obj * sizeof(Particle)));
	gpuErrchk(cudaMemcpy(cpuParticles, cpu_particles, num_CPU_obj * sizeof(Particle), cudaMemcpyHostToDevice));

	for (int i = 0; i < 10; ++i)
	{
		CollideParticles << < grid, block >> >(baumgarte_factor, num_particles, particles_ping, particles_pong, grid_cell_start, grid_cell_end, cpuParticles, num_CPU_obj);
